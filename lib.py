...
c.execute("create table if not exists issue_details(b_id varchar(5), student_id varchar(10), student_Name varchar(50) Not null, foreign key(b_id) references books(b_id))")
...
def add_book():
    ...
    sql = 'insert into books(b_id, b_name, author) values(?, ?, ?)'
    c.execute(sql, data)
    con.commit()
...
def issue_book():
    ...
    c.execute("select b_id from books where b_name = ? and available='yes'", (book,))
    book_id = c.fetchone()

    if book_id is not None:
        bid = book_id[0]
        print(bid)
        a = "insert into issue_details values(?, ?, ?)"
...
def return_book():
    ...
    c.execute("delete from issue_details where b_id = ?", (bid,))
    con.commit()
...
if user_name == 'admin' and ps == 'library123':
    print('Welcome Admin')
    while True:
        print(""" NAVEEN LIBRARY MANAGEMENT SYSTEM
